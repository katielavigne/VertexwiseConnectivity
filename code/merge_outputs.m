...
for i = 1:size(sm,1)
    for j = 1:size(nets,1)
        ...
        t.([nets{j} sm{i} '_modularity']) = gr.Q;
        t.([nets{j} sm{i} '_meanPC']) = sum(gr.PC)';
        t.([nets{j} sm{i} '_meanStrength']) = sum(gr.S)';
        t.([nets{j} sm{i} '_eigenspect']) = gr.ES;
    end
end

writetable(t,fullfile(pwd, 'vertexConnectivity_results.csv'))
